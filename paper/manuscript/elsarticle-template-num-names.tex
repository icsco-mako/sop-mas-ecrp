...
Given an agent set \deleted{$\{a_i\}_{i=0}^5$, which comprises six distinct agents: a Project Manager Agent (PM-Agent), a Data Engineer Agent (DE-Agent), an SCM Modeling Expert Agent (ME-Agent), a Python Developer Agent (PY-Agent), a Testing Engineer Agent (TE-Agent), and a Maritime Business Analyst Agent (BA-Agent).}\changed{$\mathcal{A}=\{a_{\text{PM}}, a_{\text{DE}}, a_{\text{ME}}, a_{\text{PY}}, a_{\text{TE}}, a_{\text{BA}}\}$, comprising a Project Manager Agent ($a_{\text{PM}}$), a Data Engineer Agent ($a_{\text{DE}}$), an SCM Modeling Expert Agent ($a_{\text{ME}}$), a Python Developer Agent ($a_{\text{PY}}$), a Testing Engineer Agent ($a_{\text{TE}}$), and a Maritime Business Analyst Agent ($a_{\text{BA}}$).}
...
Characterize the natural language description of the input problem as $P$, \deleted{the upper limit of selection attempts}\changed{the maximum number of agents to be selected per forward pass} as $K$, and the predetermined set of agents as \deleted{$\mathcal{A}=\{a_i\}_{i=0}^n$, where $n$ denotes the total number of agents. Proceed with the initialization of a message pool $C$, alongside the establishment of a stack $H$ to record the selected agents, adhering to a last-in-first-out (LIFO) principle to facilitate exception backtracking.} \changed{$\mathcal{A}=\{a_{\text{PM}}, a_{\text{DE}}, a_{\text{ME}}, a_{\text{PY}}, a_{\text{TE}}, a_{\text{BA}}\}$. Proceed with the initialization of a message pool $C$, alongside the establishment of a stack $H$ to record the selected agents, adhering to a last-in-first-out (LIFO) principle to facilitate exception backtracking. Throughout the algorithm, the agent subscript denotes a fixed agent identity, while $k$ ($k = 1, 2, \ldots, K$) denotes the step index within a single forward pass.}

\medskip
\noindent\textbf{Step 2: Agent Selection.} While \citet{hongMetaGPTMetaProgramming2023a} also utilizes the SOP concept, it employs a sequential approach to agent selection, whereas SOP-MAS \deleted{dynamically chooses agents according to sub-sequence rules}\changed{selects agents under an order-constrained sub-sequence rule}. Consider the executable agent set, excluding the PM-Agent itself, as the initial selectable set \deleted{$\mathcal{A}_1$=$\{a_i\}_{i=1}^5$}\changed{$\mathcal{A}_1$=$\{a_{\text{DE}}, a_{\text{ME}}, a_{\text{PY}}, a_{\text{TE}}, a_{\text{BA}}\}$} of the PM-Agent. \deleted{Denote the output at step $i$ as $y_i$, and the state as $s_i$=$(P, C, |A|-K-i)$}. \changed{Denote the output at step $k$ as $y_k$, the set of selectable agents at step $k$ as $\mathcal{A}_k$, and the state as $s_k=(P, C, K-k, \mathcal{A}_k)$}. \deleted{The policy selection function is defined as:}\changed{The PM-Agent selection function is defined as:}
\begin{equation}
...
\end{equation}
\changed{where the subscript $k$ in $y_k$ records the step position within the current forward pass}, \changed{which is in general distinct from the agent's descriptive subscript (e.g., $a_{\text{ME}}$, $a_{\text{PY}}$).}
...
C:= C\cup\{y_k\}.
\end{equation}
The sequence of remaining selectable agents is updated in accordance with the subsequence rule: \deleted{let $A_{>i}=\left(a_{i+1},\ldots,a_n\right)$ represent the remaining agents after step $i$, then set $A:= A_{>i}$.}\changed{let $\mathcal{A}^{\uparrow}=(a_{\text{DE}}, a_{\text{ME}}, a_{\text{PY}}, a_{\text{TE}}, a_{\text{BA}})$ denote the agents ordered by their fixed role sequence, $\mathcal{A}^{\uparrow}_{>a_k}$ denote the agents appearing after $a_k$ in $\mathcal{A}^{\uparrow}$, and define the selectable set for the next step as
\begin{equation}
\mathcal{A}_{k+1}:=\mathcal{A}^{\uparrow}_{>a_k}.
\end{equation}
} Finally, push $a_k$ onto the stack $H$.

\medskip
\noindent\textbf{Step 5: Iterative Modeling.}
\deleted{If the current iteration count $k$ exceeds the maximum permissible selections $K$, emit the message pool $C$ and the execution result.}\changed{The process of Steps 2--4 repeats for $k=1,2,\ldots,K$. Upon completing all $K$ steps, the message pool $C=\{y_1,y_2,\ldots,y_K\}$ captures the full collaboration chain, and the stack $H=(a_1,a_2,\ldots,a_K)$ records the selection sequence for potential backtracking. The framework then proceeds to Step~6.}

\medskip
\noindent\textbf{Step 6: Failure Detection.}
\deleted{If the TE-Agent is activated, it executes the code generated by the PY-Agent and captures the solver's runtime report (including status, objective value, constraint violations, and solution feasibility). A verifiable correctness gate is then applied:}\changed{After the $K$-step iterative modeling process, the verification stage begins. The framework first determines the verification outcome: if the TE-Agent was selected during the forward pass and the framework-level execution succeeded, the resulting objective value is reused directly; otherwise, the framework executes the latest PY-Agent solver code itself to obtain the verification outcome. The correctness gate is then applied as follows:}
\begin{itemize}
...
\end{equation}
where $\delta_k\in\{0,1\}$ is an indicator variable for whether the exception needs to be propagated upward, and $\xi_k$ represents a natural language error description for localization. \changed{If the TE-Agent was selected during this iterative modeling process, $\xi_k$ may include its structured execution report; otherwise, the diagnosis relies on the failure signal returned by the execution and verification stage. Because the forward workflow follows the monotonic order specified by $\mathcal{A}^{\uparrow}$, implementation-level defects are typically examined first at the PY-Agent, whereas failures that persist beyond code correction are propagated further to the ME-Agent for model-level revision.} If the result is correct, set $\delta_k:= 0$ and terminate the reasoning process. Otherwise, continue propagating the exception upward.
...
\caption{\changed{SOP-MAS flowchart}}\label{fig1}
...
\changed{An instance is considered correctly solved only when the generated code compiles, executes without error, attains an optimal solver status, and yields an objective value within tolerance of the ground truth, i.e.,}
\begin{equation}
...
\begin{enumerate}[label=\arabic*)]
    \item \textbf{Group~1 $(a_\text{ME}, a_\text{PY})$:} Solely comprises the ME-Agent and PY-Agent, with exception backtracking deactivated, to assess the significance of collaboration among multiple agents.

    \item \textbf{Group~2 $(a_\text{PM}, a_\text{DE}, a_\text{ME}, a_\text{PY}, a_\text{TE})$:} Excluding the BA-Agent whilst permitting exception backtracking to examine the role of the BA-Agent.

    \item \textbf{Group~3 $(a_\text{PM}, a_\text{ME}, a_\text{PY}, a_\text{TE}, a_\text{BA})$:} The DE-Agent is excluded, with exception backtracking enabled, to evaluate the significance of structured modeling information.

    \item \textbf{Group~4 $(a_\text{PM}, a_\text{DE}, a_\text{ME}, a_\text{PY}, a_\text{BA})$:} The TE-Agent is omitted to examine the interaction between testing and exception backtracking, while exception backtracking remains enabled.
...
The other agents, including the DE-Agent (8.38 ± 1.43 seconds), ME-Agent (9.21 ± 1.49 seconds), and BA-Agent (6.79 ± 1.45 seconds), also demonstrate efficient performance with acceptable variances, contributing to the overall robustness and practical deployability of the SOP-MAS framework.
...
\begin{table}[htbp]
\centering
\caption{\changed{Comparison of cost structures under the two scenarios (CNY)}}
...
\footnotesize
\textit{Note:} Scenario~1 permits hinterland lateral repositioning; Scenario~2 disables inter-dry-port exchange.
\end{table}
...
\end{lstlisting}
\end{promptbox}
\captionof{lstlisting}{Representative prompt template for the ME-Agent}
